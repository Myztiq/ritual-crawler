  ...
  EntityPlayer = ig.Box2DEntity.extend({
    ...
    addToSpellBook: function (spell) {
      ...
      spell.comboHandle = this.comboManager.add(spell.comboButtons, spell.time, function () {
        ...
        $('.player' + self.playerName + ' .' + spell.id).parent().parent().addClass('active-spell');
  ...
  EntityBlood = ig.Box2DEntity.extend({
    ...
    init: function (x, y, settings) {
      ...
      if (settings.playerName === 'B') {
        if (Math.random() < .5){
          this.addAnim('idle', .1, [3 * 8 + 2]);
        } else {
          this.addAnim('idle', .1, [3 * 8 + 3]);
        }
      } else {
        if (Math.random() < .5){
          this.addAnim('idle', .1, [2 * 8 + 2]);
        } else {
          this.addAnim('idle', .1, [2 * 8 + 3]);
        }
      }
    },
    getBody: function (){
