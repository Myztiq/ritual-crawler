  ...
  EntityPlayer = ig.Box2DEntity.extend({
    ...
    kill: function () {
      ...
      if (self.killme) {
        ...
        var respawnCombo = [];
        var savingThrowCombo = [buttons.random()];
        ...
        savingThrowCombo.forEach((button) => {
          savingThrowComboImgs += `<img src="media/buttons/${button[this.gamepadType]}" height="32" width="32"/>`
        });

        $('.respawnPlayer' + this.playerName).append(`<div class="respawn">${respawnComboImgs}</div>`);
        ...
        ig.game.spawnEntity(EntityCorpse, self.pos.x, self.pos.y, {
          ...
          keys: savingThrowKeys,
          comboImg: savingThrowComboImgs
        });
    ...
    draw: function () {
      ...
      ig.system.context.rect(
        ...
        ((this.size.x - 2) * (this.armor / (this.armorMax + 4))) * ig.system.scale * 3,
        2 * ig.system.scale
      );
      ig.system.context.closePath();
      ig.system.context.fill();
