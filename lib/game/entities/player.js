...
.defines(function () {
  EntityPlayer = ig.Box2DEntity.extend({
    ...
    init: function (x, y, settings) {
      ...
      this.addAnim('walkDownRight', 0.2, [1,2,3]);
      this.addAnim('walkUp', 0.2, [4,5]);
      this.addAnim('walkDownLeft', 0.2, [6,7,8]);
    ...
    kill: function () {
      ...
      if (self.killme) {
        ...
        var savingThrowHandle = ig.game[key].add(savingThrow, 999999, function () {
          console.log('remove', '.respawnPlayer' + self.playerName + ' .respawn')
      ...
      $('.player' + this.playerName + ' .spellbook').append(
        ...
        '<div class="content">' +
          '<span>' +
            ...
            `<h4 class="spell-level">${spell.level}</h4>` +
        '</span>' +
        `<span class="spell-info ${spell.id}">${spell.name} Level: ${spell.level}</span>` +
