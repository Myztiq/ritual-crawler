ig.module(
  ...
  .defines(function () {
    ...
    EntityAttackHoming = ig.Box2DEntity.extend({
      ...
      animSheet: new ig.AnimationSheet('media/bee.png', 32, 32),
      flip: true,
      ...
      init: function (x, y, settings) {
        ...
        var initialAngle = Math.atan2(this.target.pos.y - y, this.target.pos.x - x);

        if (!settings.spawningChild) {
          initialAngle += Math.PI;
          ig.game.spawnEntity(EntityAttackHoming, x, y, {
            spawningChild: false
          });
        }

        var dx = Math.cos(initialAngle) * 10;
        var dy = Math.sin(initialAngle) * 10;
